...
/** \brief tries to set the scheduler to FIFO
  * \param nice the priority will be (max_priority - nice). Do NOT set everybody to nice==0. 
  * 
  * @return 0 if OK, the apropriate error code from posix API if an error occurred
...
int Ut::goRealTime(int nice) {
  ...
  // gets thread parameters from the SO for the current thread
  struct sched_param proc_sched;
  if (sched_getparam(0, &proc_sched)) { // 0 means the current thread
      ROS_ERROR_THROTTLE (5,"Ut::goRealTime() Could not get thread parameters: %s", strerror (errno));
    return errno;
  }
  // change the priority to (the maximum priority - nice). bigger values = more priority
  ...
  if (0 == sched_setscheduler (0, SCHED_FIFO, &proc_sched)) {
      ...
      ROS_ERROR_THROTTLE(5,"Ut::goRealTime() failed to set real-time scheduler: %s", strerror (errno));
      return errno;
  }
...
/// return true if the current thread use the FIFO scheduler
bool Ut::isRT() {
    return SCHED_FIFO == sched_getscheduler(0);
}
