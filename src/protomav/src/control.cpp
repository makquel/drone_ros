 ...
 switch(item.command){
   case(MAV_CMD_NAV_TAKEOFF):
    ...
    if(item.frame == MAV_FRAME_GLOBAL_RELATIVE_ALT){
      ROS_INFO("MAV_FRAME_GLOBAL_RELATIVE_ALT selected");
      ...
      ROS_ERROR("Take-off handle not implemented");
      return false;
    }
    ROS_INFO("WP w: %f", wp->getPose().pose.orientation.w);
    wpm->push(*wp);
   ...
   if(item.frame == MAV_FRAME_GLOBAL){
     ...
     ROS_ERROR("Landing handle not implemented");
     return false;
   }
   ROS_INFO("WP w: %f", wp->getPose().pose.orientation.w);
   wpm->push(*wp);
