...
void GrabCallbackClass::PrintCameraInfo( )
{
  ...
  int GrabCallbackClass::RunSingleCamera( 
    ...
    , double setframerate, double shutter
    )
  {
    ...
    if (setframerate>0) 
        setProperty(FRAME_RATE,false,setframerate);
    if (shutter>0) 
        setProperty(SHUTTER,false,shutter);
    
    // Start capturing images
    error = cam.StartCapture(callback);
    if (PrintError())  OnError();
...
int GrabCallbackClass::PrintError( )
  {
    if (error !=PGRERROR_OK) {
...
void GrabCallbackClass::OnError( )
  ...
  bool GrabCallbackClass::setProperty(const FlyCapture2::PropertyType &type, const bool &autoSet, double &value)
  {
