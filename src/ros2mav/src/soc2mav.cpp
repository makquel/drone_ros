...
void SoC2Mav::send(){
  ...
  ///mavlink_msg_battery_status_encode( system_id,  comp_id, &mmsg,  &mav_soc2);
...
void SoC2Mav::voltageCallback(const yocto::voltage_info::ConstPtr& voltage){
  ...
  for (int i = 0; i < 10; i++){
    mav_soc.voltages[i] = 0.0;
  }
  mav_soc.voltages[0] = voltage->current_value_1*10e3;
  mav_soc.voltages[1] = voltage->current_value_2*10e3;
  mav_soc.voltages[2] = voltage->current_value_3*10e3;
  mav_soc.voltages[3] = voltage->current_value_4*10e3;
